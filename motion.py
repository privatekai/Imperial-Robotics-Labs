...
DISTANCE_ERROR = -0.84 * PI # Making it bigger makes it go less far
ANGLE_ERROR = 6.16 # Making it bigger makes it turn more
...
class Motion:
    def __init__(self, bp_instance, left_port=None, right_port=None):
        ...
        self.BP.set_motor_limits(self.left_port,  50, MOVEMENT_SPEED)
        self.BP.set_motor_limits(self.right_port, 50, MOVEMENT_SPEED)

    def _wait_for_motor_position(self, left_target, right_target):
        """
        ...
        while time.time() - start_time < TIMEOUT:
            try:
                left_current = self.BP.get_motor_encoder(self.left_port)
                right_current = self.BP.get_motor_encoder(self.right_port)
                ...
                print("IOError in _wait_for_motor_position: %s" % error)
                time.sleep(0.1)
    ...
    def forward(self, distance: float, particles=None):
        ...
        try:
            ...
            self.BP.set_motor_position(self.left_port, target)
            self.BP.set_motor_position(self.right_port, target)

            self._wait_for_motor_position(target, target)
    ...
    def turnAntiClockwise(self, angle: float, particles=None):
        """
        ...
        try:
            ...
            self.BP.set_motor_limits(self.left_port, 50, TURNING_SPEED)
            self.BP.set_motor_limits(self.right_port, 50, TURNING_SPEED)
            ...
            self.BP.set_motor_position(self.left_port, left_target)
            self.BP.set_motor_position(self.right_port, right_target)

            self._wait_for_motor_position(left_target, right_target)
...
if __name__ == "__main__":
    ...
    def calibrate_angle():
        ...
        motion.turnAntiClockwise(angle_deg, dummy)
